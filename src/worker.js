...
const testBot=async()=>{try{const r=await fetch('/api/test-telegram',{method:'POST',headers:{'Authorization':'Bearer '+localStorage.getItem('token')}});const d=await r.json();if(d.success){alert("✅ 测试通知发送成功！\n\n🎉 恭喜，Bot已经连接成功啦！\n\n发送 /help 或者 /start 查看可用命令");}else{alert("❌ 测试通知发送失败！\n\n错误信息: "+(d.error||d.message||"未知错误")+"\n\n请检查: \n1. TELEGRAM_BOT_TOKEN 是否正确\n2. TELEGRAM_CHAT_ID 是否正确\n3. 是否已向Bot发送过消息");}}catch(e){alert("❌ 测试失败！\n\n网络错误，请检查网络连接后重试");}};
